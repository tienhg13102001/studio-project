...
const DANH_SACH: Nap[] = [
  ...
  {
    vi: "Lần đầu bên chị làm thử một bộ thôi, xong thấy dùng được cho cả sàn thương mại điện tử lẫn ảnh treo cửa hàng nên gọi lại luôn lần hai. Lookbook thì nhiều bên chụp đẹp, nhưng đẹp mà nhìn không ra chất vải thì bên bán hàng của chị không dùng được. Chỗ đó Bee Z làm được.",
    en: "The first time we only tried one set. It ended up working for both the e-commerce listings and the in-store prints, so we called them back for a second. Plenty of studios shoot a beautiful lookbook — but if you cannot read the fabric, my sales team cannot use it. That is the part Bee Z gets right.",
    ten: "Như Quỳnh",
  ...
  {
    vi: "Bọn mình quay theo đợt, mỗi đợt một nội dung khác nhau, nên thứ mệt nhất là phải giải thích lại từ đầu mỗi lần đổi ê-kíp. Với Bee Z thì từ đợt hai trở đi gần như chỉ cần gửi brief là chạy. Riêng khoản đó đã tiết kiệm cho bọn mình khá nhiều thời gian họp.",
  ...
  {
    vi: "Bọn em xem showreel thì thích lắm, nhưng vẫn lo hôm đó đông ê-kíp quá sẽ mệt và mất tự nhiên. Thực tế thì gần như không thấy các bạn ấy đâu, mà lúc xem phim lại có đủ những đoạn em không hề biết là có người đang quay. Đoạn mẹ em lau nước mắt lúc chưa vào lễ — cả nhà xem đi xem lại.",
...
async function chay(): Promise<void> {
  ...
  for (const [i, n] of DANH_SACH.entries()) {
    const daCo = await Testimonial.findOne({ "quote.vi": n.vi });
    if (daCo) {
      boQua++;
